...
`popSe.eventMap`は差分保存で、`getEventMap()`が現在のプリセットのデフォルトに重ねて全30イベント分を返す。
...
- AIアシスタント (Claude Code / Codex / Copilot) 検出は二系統:
  ...
  - Copilotのインライン補完/チャット応答は公開APIで検出不可 (既知の制限)
...
- AIアシスタントトリガー6イベント追加 (Claude Code / Codex / GitHub Copilot の開始・終了、計30イベント)
- tsc strictでコンパイル成功
