...
**応答出力中 (`aiOutput`) の自動検出**:

- Copilot Chat 等がチャット応答内の**コードブロックをストリーム**すると、チャット系仮想ドキュメント (`vscode-chat-code-block` 等) の変更イベントが発火するため、これを検出して鳴らします
- AIエージェント (Copilot Edits / Claude Code / Codex) が**アクティブエディタ以外の開いているファイルを編集**したときも鳴らします (ユーザーのタイプはアクティブエディタで起きるため干渉しません)
- ストリーミング中は250ms間隔に間引かれ、カタカタというティック音になります (デフォルト音はタイプ音系の控えめな音)
- 注意: ヒューリスティックのため、git操作や一括置換など「AI以外の非アクティブファイル変更」でも鳴ることがあります。不要なら Event Mapping で無音にしてください
