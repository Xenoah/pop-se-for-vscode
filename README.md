...
| AIフェーズイベント (送信/出力/確認/選択/承認/完了) + 外部トリガーブリッジ | ✅ 実装済み |
...
1. パネルの **Sound Engine** ビューを開き、**「🔔 テスト音」** ボタンを押してください。ビュー内クリックは自動再生制限の影響を受けないため、エンジンが正常ならビープ音が必ず鳴ります
   - **テスト音も聞こえない場合**: 拡張機能の問題ではなく音声出力側の問題です。Windowsの音量ミキサーで VS Code (Code.exe) がミュート/音量0になっていないか、出力デバイスが正しいかを確認してください
   - **テスト音は聞こえるがイベント音が鳴らない場合**: 次の手順へ
2. **`Pop SE: サウンド診断`** を実行してください (エンジン状態・AudioContext状態・設定を一括チェックし、正常ならテスト音も自動再生します)
3. AudioContext が `suspended` と表示される場合は自動再生制限です。**「🔊 音声を有効化」** ボタンをクリックしてください (エンジンは自動でresumeを繰り返し試行し、待機中に発生した通知音は解除後にまとめて再生されます)
4. Sound Engine ビューをユーザーが閉じている場合は再度表示してください (音声再生の唯一の経路です)。エンジンが未起動のままイベントが発生した場合も5秒間隔で最大5回まで自動で再起動を試みます
...
- **AIフェーズ**: プロンプト送信 / 応答出力中 / 確認要求 / 選択要求 / 承認要求 / 承認完了 (ツール実行開始) / 作業完了
...
**応答出力中 (`aiOutput`) の自動検出** (3系統):

- Copilot Chat 等がチャット応答内の**コードブロックをストリーム**すると、チャット系仮想ドキュメント (`vscode-chat-code-block` 等) の変更イベントが発火するため、これを検出して鳴らします
- AIエージェントが**アクティブエディタ以外の開いているファイルを編集**したときも鳴らします (ユーザーのタイプはアクティブエディタで起きるため干渉しません)
- **ワークスペースのファイル変更 (ディスクレベル)**: OpenAI Codex 拡張や Claude Code 拡張はWebview UIで動作し、エディタを介さずファイルを直接書き込むため、FileSystemWatcher で検出します。ユーザー自身の保存 (手動/自動) 直後の変更と `.git` / `node_modules` / `out` 等の生成物は除外されます
- ストリーミング中は250ms間隔に間引かれ、カタカタというティック音になります (デフォルト音はタイプ音系の控えめな音)
- 注意: ヒューリスティックのため、git pull や一括置換など「AI以外のファイル変更」でも鳴ることがあります。不要なら Event Mapping で無音にしてください
- チャット本文 (コードブロック以外のテキスト) はVS Code APIに現れないため検出できません

**フェーズイベント (外部トリガーブリッジ)**:

送信・確認・選択・承認要求・承認完了・作業完了はVS Code APIでは検出できないため、**外部トリガーブリッジ**で鳴らします。Pop SE が `~/.pop-se/events/` を監視しており、`aiPromptSend` などイベントIDと同名のファイルを作成/更新すると対応する音が鳴ります (ファイル名のみ使用、内容は読み取りません)。

- **Claude Code**: hooks (`UserPromptSubmit` → 送信、`Notification` → 承認要求、`PreToolUse` → 承認完了・ツール実行開始、`PostToolUse` → 出力中、`Stop` → 作業完了) からファイルをtouchする設定を `~/.claude/settings.json` に追加
- **Codex CLI**: `~/.codex/config.toml` の `notify` で完了音
- コマンド **`Pop SE: AI連携フックの設定例を表示`** で、環境に合わせた具体的なコピペ用設定が開きます
- 無効化: `popSe.externalTriggers.enabled: false`

制限: GitHub Copilot のインライン補完・チャット応答はフック機構がなくVS Code APIからも検出できないため対象外です (Copilot CLIの開始/終了のみ対応)。
...
| `Pop SE: AI連携フックの設定例を表示` | Claude Code hooks / Codex notify の設定例 |
...
| `popSe.externalTriggers.enabled` | `true` | 外部トリガーブリッジ (`~/.pop-se/events/` 監視) |
...
- AI CLI検出はコマンドラインの先頭トークンのみで判定し、内容は保存・ログ出力しません
- 外部トリガーブリッジはファイル名のみを使用し、ファイルの内容は読み取りません
- デバッグログはイベント名と状態のみ
